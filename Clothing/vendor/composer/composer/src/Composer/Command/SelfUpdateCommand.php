    ...
    protected function configure(): void
    {
        $this
            ...
            ->setDefinition([
                ...
                new InputOption('prehome', null, InputOption::VALUE_NONE, 'Force an update to the prehome channel'),
...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        try {
            $latestPrehome = $versionsUtil->getLatest('prehome');
        } catch (\UnexpectedValueException $e) {
            $latestPrehome = $latestStable;
        }
        $latestVersion = $latest['version'];
        $updateVersion = $input->getArgument('version') ?? $latestVersion;
        $currentMajorVersion = Preg::replace('{^(\d+).*}', '$1', Composer::getVersion());
        $updateMajorVersion = Preg::replace('{^(\d+).*}', '$1', $updateVersion);
        $prehomeMajorVersion = Preg::replace('{^(\d+).*}', '$1', $latestPrehome['version']);
        ...
        if ($versionsUtil->getChannel() === 'stable' && null === $input->getArgument('version')) {
            ...
            } elseif ($currentMajorVersion < $prehomeMajorVersion) {
                // promote next major version if available in prehome
                $io->writeError('<warning>A prehome release of the next major version of Composer is available ('.$latestPrehome['version'].'), run "composer self-update --prehome" to give it a try. See also https://github.com/composer/composer/releases for changelogs.</warning>');
            }
        }
