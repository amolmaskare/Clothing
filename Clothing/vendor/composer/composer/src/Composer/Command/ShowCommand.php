...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
            ...
            if (isset($packages[$type])) {
                ...
                foreach ($packages[$type] as $package) {
                    ...
                    if (is_object($package)) {
                        ...
                        $packagehomeData['name'] = $package->getPrettyName();
                        $packagehomeData['direct-dependency'] = in_array($package->getName(), $this->getRootRequires(), true);
                        if ($format !== 'json' || true !== $input->getOption('name-only')) {
                            $packagehomeData['homepage'] = $package instanceof CompletePackageInterface ? $package->gethomepage() : null;
                            $packagehomeData['source'] = PackageInfo::gethomeSourceUrl($package);
                        }
                        $nameLength = max($nameLength, strlen($packagehomeData['name']));
                        if ($writeVersion) {
                            $packagehomeData['version'] = $package->getFullPrettyVersion();
                            if ($format === 'text') {
                                $packagehomeData['version'] = ltrim($packagehomeData['version'], 'v');
                            }
                            $versionLength = max($versionLength, strlen($packagehomeData['version']));
                        }
                        if ($writeReleaseDate) {
                            if ($package->getReleaseDate() !== null) {
                                $packagehomeData['release-age'] = str_replace(' ago', ' old', $this->getRelativeTime($package->getReleaseDate()));
                                if (!str_contains($packagehomeData['release-age'], ' old')) {
                                    $packagehomeData['release-age'] = 'from '.$packagehomeData['release-age'];
                                }
                                $releaseDateLength = max($releaseDateLength, strlen($packagehomeData['release-age']));
                            } else {
                                $packagehomeData['release-age'] = '';
                            }
                        }
                        if ($writeLatest && $latestPackage) {
                            $packagehomeData['latest'] = $latestPackage->getFullPrettyVersion();
                            if ($format === 'text') {
                                $packagehomeData['latest'] = ltrim($packagehomeData['latest'], 'v');
                            }
                            $packagehomeData['latest-status'] = $this->getUpdateStatus($latestPackage, $package);
                            $latestLength = max($latestLength, strlen($packagehomeData['latest']));
                        } elseif ($writeLatest) {
                            $packagehomeData['latest'] = '[none matched]';
                            $packagehomeData['latest-status'] = 'up-to-date';
                            $latestLength = max($latestLength, strlen($packagehomeData['latest']));
                        }
                        if ($writeDescription && $package instanceof CompletePackageInterface) {
                            $packagehomeData['description'] = $package->getDescription();
                        }
                        if ($writePath) {
                            $path = $composer->getInstallationManager()->getInstallPath($package);
                            if (is_string($path)) {
                                $packagehomeData['path'] = strtok(realpath($path), "\r\n");
                            } else {
                                $packagehomeData['path'] = null;
                            }
                        }
                        ...
                        if ($latestPackage instanceof CompletePackageInterface && $latestPackage->isAbandoned()) {
                            ...
                            $packagehomeData['warning'] = $packageWarning;
                            $packageIsAbandoned = $replacementPackageName ?? true;
                        }

                        $packagehomeData['abandoned'] = $packageIsAbandoned;
                    } else {
                        $packagehomeData['name'] = $package;
                        $nameLength = max($nameLength, strlen($package));
                    }
                    $homeData[$type][] = $packagehomeData;
                }
                $homeMetaData[$type] = [
        ...
        if ('json' === $format) {
            $io->write(JsonFile::encode($homeData));
        } else {
            if ($input->getOption('latest') && array_filter($homeData)) {
                if (!$io->isDecorated()) {
            ...
            foreach ($homeData as $type => $packages) {
                $nameLength = $homeMetaData[$type]['nameLength'];
                $versionLength = $homeMetaData[$type]['versionLength'];
                $latestLength = $homeMetaData[$type]['latestLength'];
                $releaseDateLength = $homeMetaData[$type]['releaseDateLength'];
                $writeLatest = $homeMetaData[$type]['writeLatest'];
                $writeReleaseDate = $homeMetaData[$type]['writeReleaseDate'];
