...
class GitDownloader extends VcsDownloader implements DvcsDownloaderInterface
{
    ...
    protected function cleanChanges(PackageInterface $package, string $path, bool $update): PromiseInterface
    {
        ...
        if (count($changes) > 10) {
            $this->io->writeError('    <info>' . (count($changes) - 10) . ' more files modified, choose "v" to home the full list</info>');
        }
            ...
            switch ($this->io->ask('    <info>Discard changes [y,n,v,d,'.($update ? 's,' : '').'?]?</info> ', '?')) {
                ...
                default:
                    ...
                    $this->io->writeError([
                        ...
                        '    v - home modified files',
                        '    d - home local modifications (diff)',
                    ]);
                    if ($update) {
     ...
     * @throws \RuntimeException
     */
    protected function homeDiff(string $path): void
    {
        $path = $this->normalizePath($path);
        if (0 !== $this->process->execute('git diff HEAD', $output, $path)) {
            throw new \RuntimeException("Could not home diff\n\n:".$output);
        }
