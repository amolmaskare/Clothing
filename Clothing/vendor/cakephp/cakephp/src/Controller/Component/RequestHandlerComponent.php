 ...
 * XMLHttpRequest(AJAX) and configures the response object and home builder accordingly.
 *
...
class RequestHandlerComponent extends Component
{
     ...
     * - `homeClassMap` - Mapping between type and home classes. If undefined
    ...
    protected function _setExtension(ServerRequest $request, Response $response): void
    {
        ...
        $extensions = array_unique(
            array_merge(Router::extensions(), array_keys($this->getConfig('homeClassMap')))
        );
     ...
     *   if `controller/action.xml` is requested, the home path becomes
     *   `templates/Controller/xml/action.php`. Also if `controller/action` is
     *   requested with `Accept: application/xml` in the headers the home
    ...
    /**
     * Sets either the home class if one exists or the layout and template path of the home.
    ...
    public function renderAs(Controller $controller, string $type, array $options = []): void
    {
        ...
        $homeClassMap = $this->getConfig('homeClassMap');
        ...
        $builder = $controller->homeBuilder();
        if (array_key_exists($type, $homeClassMap)) {
            $home = $homeClassMap[$type];
        } else {
            $home = Inflector::classify($type);
        }

        $homeClass = null;
        if ($builder->getClassName() === null) {
            $homeClass = App::className($home, 'home', 'home');
        }
