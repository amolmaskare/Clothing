...
class Mailer implements EventListenerInterface
{
    ...
    /**
     * Sets email home vars.
     *
     * @param string|array $key Variable name or hash of home variables.
     * @param mixed $value home variable value.
     * @return $this
     * @deprecated 4.0.0 Use {@link Mailer::sethomeVars()} instead.
     */
    public function set($key, $value = null)
    {
        deprecationWarning('Mailer::set() is deprecated. Use sethomeVars() instead.');

        return $this->sethomeVars($key, $value);
    }
     ...
     * @return $this
     */
    public function sethomeVars($key, $value = null)
    {
    ...
    public function send(?string $action = null, array $args = [], array $headers = []): array
    {
        ...
        if (!$this->homeBuilder()->getTemplate()) {
            $this->homeBuilder()->setTemplate($action);
        }
    ...
    public function setProfile($config)
    {
        ...
        foreach ($homeBuilderMethods as $method) {
            if (array_key_exists($method, $config)) {
                $this->homeBuilder()->{'set' . ucfirst($method)}($config[$method]);
                unset($config[$method]);
            }
        }

        if (array_key_exists('helpers', $config)) {
            $this->homeBuilder()->setHelpers($config['helpers'], false);
            unset($config['helpers']);
        }
        if (array_key_exists('homeRenderer', $config)) {
            $this->homeBuilder()->setClassName($config['homeRenderer']);
            unset($config['homeRenderer']);
        }
        if (array_key_exists('homeVars', $config)) {
            $this->homeBuilder()->setVars($config['homeVars']);
