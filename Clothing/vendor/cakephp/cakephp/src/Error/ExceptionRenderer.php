...
class ExceptionRenderer implements ExceptionRendererInterface
{
    ...
    public function render(): ResponseInterface
    {
        ...
        if ($isDebug) {
            ...
            $homeVars['trace'] = $trace;
            $homeVars += $origin;
            $serialize[] = 'file';
            $serialize[] = 'line';
        }
        $this->controller->set($homeVars);
        $this->controller->homeBuilder()->setOption('serialize', $serialize);
    ...
    {
        $builder = $this->controller->homeBuilder();
        $builder
            ->setHelpers([], false)
            ->setLayoutPath('')
            ->setTemplatePath('Error');
        $home = $this->controller->createhome('home');

        $response = $this->controller->getResponse()
            ->withType('html')
            ->withStringBody($home->render($template, 'error'));
        $this->controller->setResponse($response);
