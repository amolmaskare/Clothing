...
use Cake\TestSuite\Constraint\home\LayoutFileEquals;
use Cake\TestSuite\Constraint\home\TemplateFileEquals;
...
trait IntegrationTestTrait
{
    ...
    /**
     * Adds additional event spies to the controller/home event manager.
     *
    ...
    public function controllerSpy(EventInterface $event, ?Controller $controller = null): void
    {
        ...
        $events->on('home.beforeRender', function ($event, $homeFile): void {
            if (!$this->_homeName) {
                $this->_homeName = $homeFile;
            }
        });
        $events->on('home.beforeLayout', function ($event, $homeFile): void {
            $this->_layoutName = $homeFile;
        });
    }
    ...
    /**
     * Fetches a home variable by name.
     *
     * If the home variable does not exist, null will be returned.
     *
     * @param string $name The home variable to get.
     * @return mixed The home variable if set.
     */
    public function homeVariable(string $name)
    {
        return $this->_controller ? $this->_controller->homeBuilder()->getVar($name) : null;
    }
    ...
    {
        $verboseMessage = $this->extractVerboseMessage($message);
        $this->assertThat($content, new TemplateFileEquals($this->_homeName), $verboseMessage);
    }
    ...
    protected function extractVerboseMessage(string $message): string
    {
        ...
        $error = $this->_controller->homeBuilder()->getVar('error');
        if ($error instanceof Exception) {
            $message .= $this->extractExceptionMessage($this->homeVariable('error'));
        }
