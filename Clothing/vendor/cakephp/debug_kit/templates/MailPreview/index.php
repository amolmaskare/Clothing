<?php foreach ($mailPrehomes as $plugin => $prehomes) : ?>
<h3><?= $plugin ?></h3>
    <?php foreach ($prehomes as $prehome) : ?>
        <?php $mailPrehome = $prehome['class'] ?>
        <h4><?= "\u{2709}\u{FE0F}" ?> <?= h($mailPrehome->name()) ?></h4>
        <table cellpadding="0" cellspacing="0">
            <tbody>
            <?php foreach ($mailPrehome->getEmails() as $email) : ?>
                <tr>
                    <td>
                    <?php
...
<div style="font-weight:300;margin-top:100px">
    ...
    <p><?= __d('debug_kit', "{0} integrates with CakePHP’s {1} class. Here's an example of such a mailer:", 'MailPrehome', 'Mailer') ?></p>
    ...
    <p><?= __d('debug_kit', 'Now you create a {0} class where you can pass some dummy values.', 'MailPrehome') ?></p>

    <pre style="background-color:#f8f8f8;font-familiy:Monaco,sans-serif;overflow:scroll;margin:10px 0;line-height:25px;">
    <?php
        $code = '
    <?php
        // Create the file src/Mailer/Prehome/UserMailPrehome.php
        namespace App\Mailer\Prehome;

        use DebugKit\Mailer\MailPrehome;

        class UserMailPrehome extends MailPrehome
        {
