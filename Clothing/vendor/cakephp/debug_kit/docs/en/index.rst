...
has been installed and loaded. Each panel is comprised of a panel class and home
element. Typically, a panel handles the collection and display of a single type
of information such as Logs or Request information. You can choose to home panels
...
* **History** Displays a list of previous requests, and allows you to load
  and home toolbar data from previous requests.
* **Include** home the included files grouped by type.
...
* **Mail** Display all emails sent during the request and allow to prehome
...
* **Variables** Display home variables set in controller.
...
DebugKit. It provides a way to home toolbar data from previous requests,
...
The mailer prehome allows you to easily check emails during development.
...
Creating Prehome Classes
------------------------

In order to prehome emails before sending them, you need to create a prehome
class that defines the receipient and required template variables for your
mailer methods::

    // in src/Mailer/Prehome/WelcomePrehome.php
...
MailPrehome classes should live in the ``Mailer\Prehome`` namespace of your
application or plugin, and use the ``Prehome`` class suffix.
...
Each Panel is expected to have a home element that renders the content from the
