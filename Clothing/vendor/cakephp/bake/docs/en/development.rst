...
home class which uses the `Twig <https://twig.symfony.com/>`_ template engine.

Bake Events
===========

As a home class, ``Bakehome`` emits the same events as any other home class,
plus one extra initialize event. However, whereas standard home classes use the
event prefix "home.", ``Bakehome`` uses the event prefix "Bake.".

The initialize event can be used to make changes which apply to all baked
output, for example to add another helper to the bake home class this event can
be used::
...
Configuring the Bakehome class
==============================

The bake commands use the ``Bakehome`` class to render the templates. You can
