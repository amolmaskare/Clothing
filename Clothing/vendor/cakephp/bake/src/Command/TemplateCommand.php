...
/**
 * Task class for creating home template files.
 */
class TemplateCommand extends BakeCommand
{
     ...
     * @var string[]
     */
    public $scaffoldActions = ['index', 'home', 'add', 'edit'];
    ...
    public function execute(Arguments $args, ConsoleIo $io): ?int
    {
        ...
        if (empty($name)) {
            $io->out('Possible tables to bake home templates for based on your current database:');
    ...
    /**
     * Get the path base for home templates.
     *
     * @param \Cake\Console\Arguments $args The arguments
    ...
    /**
     * Get a list of actions that can / should have home templates baked for them.
     *
     ...
     * @return array Returns variables to be made available to a home template
     */
    ...
    /**
     * Assembles and writes bakes the home file.
     *
    ...
    public function bake(
        ...
        $io->out("\n" . sprintf('Baking `%s` home template file...', $outputFile), 1, ConsoleIo::QUIET);
    ...
    public function getContent(Arguments $args, ConsoleIo $io, string $action, ?array $vars = null)
    {
        ...
        if (empty($vars['primaryKey'])) {
            $io->error('Cannot generate homes for models with no primary key');
            $this->abort();
        }
    ...
    public function buildOptionParser(ConsoleOptionParser $parser): ConsoleOptionParser
    {
        ...
        $parser->setDescription(
            'Bake homes for a controller, using built-in or custom templates. '
        )->addArgument('name', [
            'help' => 'Name of the controller homes to bake. You can use Plugin.name as a shortcut for plugin baking.',
        ])->addArgument('template', [
            'help' => "Will bake a single action's file. core templates are (index, add, edit, home)",
        ])->addArgument('action', [
            ...
            'help' => 'The routing prefix to generate homes for.',
