...
namespace Cake\Twighome\home;

use Cake\Core\Configure;
use Cake\Core\Plugin;
use Cake\Twighome\Panel\TwigPanel;
use Cake\Twighome\Twig\Extension;
use Cake\Twighome\Twig\FileLoader;
use Cake\Twighome\Twig\TokenParser;
...
class Twighome extends home
{
    ...
    protected function initializeExtensions(): void
    {
        ...
        $twig->addExtension(new Extension\homeExtension());
    ...
    {
        // Set _home for each render because Twig Environment is shared between homes.
        $this->getTwig()->addGlobal('_home', $this);

        $dataForhome = array_merge(
            $dataForhome,
            iterator_to_array($this->helpers()->getIterator())
        );

        return $this->getTwig()->load($templateFile)->render($dataForhome);
    }
