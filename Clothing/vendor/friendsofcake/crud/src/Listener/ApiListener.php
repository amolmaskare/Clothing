...
class ApiListener extends BaseListener
{
    ...
    /**
     * Selects an specific Crud home class to render the output
     *
    ...
    /**
     * Ensure _serialize is set in the home
     *
     * @return void
     */
    protected function _ensureSerialize(): void
    {
        ...
        if ($controller->homeBuilder()->getOption('serialize') !== null) {
            return;
        }
        ...
        $action = $this->_action();
        if (method_exists($action, 'homeVar')) {
            $serialize['data'] = $action->homeVar();
        } else {
            $serialize[] = 'data';
        }

        $serialize = array_merge($serialize, (array)$action->getConfig('serialize'));
        $controller->homeBuilder()->setOption('serialize', $serialize);
    }

    /**
     * Ensure success key is present in Controller::$homeVars
     *
     * @param \Crud\Event\Subject $subject Subject
     * @return void
     */
    protected function _ensureSuccess(Subject $subject): void
    {
        ...
        if ($controller->homeBuilder()->getVar('success') !== null) {
            return;
        }
    ...
    /**
     * Ensure data key is present in Controller:$homeVars
     *
     * @param \Crud\Event\Subject $subject Subject
     * @return void
     */
    protected function _ensureData(Subject $subject): void
    {
        ...
        if (method_exists($action, 'homeVar')) {
            $homeVar = $action->homeVar();
        } else {
            $homeVar = 'data';
        }

        if ($controller->homeBuilder()->getVar($homeVar) !== null) {
            return;
        }
    ...
    /**
     * Inject home classes into RequestHandler
     *
     * @return void
     */
    public function injecthomeClasses(): void
    {
        $controller = $this->_controller();
        foreach ($this->getConfig('homeClasses') as $type => $class) {
            $controller->RequestHandler->setConfig('homeClassMap', [$type => $class]);
        }
    }

    /**
     * Get or set a homeClass
     *
     * `$type` could be `json`, `xml` or any other valid type
     *      defined by the `RequestHandler`
     *
     * `$class` could be any home class capable of handling
     ...
     * @return mixed
     */
    public function homeClass(string $type, ?string $class = null)
    {
        if ($class === null) {
            return $this->getConfig('homeClasses.' . $type);
        }

        return $this->setConfig('homeClasses.' . $type, $class);
    }
