...
class RelatedModelsListener extends BaseListener
{
    ...
    /**
     * Fetches related models' list and sets them to a variable for the home
     *
    ...
    /**
     * Find and publish all related models to the home
     * for an action
     *
    ...
    public function publishRelatedModels(?string $action = null, ?EntityInterface $entity = null): void
    {
        ...
        foreach ($models as $name => $association) {
            ...
            $homeVar = Inflector::variable($associationName);
            if (array_key_exists($homeVar, $controller->homeBuilder()->getVars())) {
                continue;
            }

            $finder = $this->finder($association);
            $query = $association->find($finder, $this->_findOptions($association));
            $subject = $this->_subject(compact('name', 'homeVar', 'query', 'association', 'entity'));
            $event = $this->_trigger('relatedModel', $subject);

            $controller->set($event->getSubject()->homeVar, $event->getSubject()->query->toArray());
        }
    }
