...
class CrudComponent extends Component
{
    ...
    public function execute(?string $controllerAction = null, array $args = []): ResponseInterface
    {
        ...
        if (method_exists($crudAction, 'home')) {
            $home = $crudAction->home();
        }

        return $this->_controller->render($home);
    }
    ...
    /**
     * Map the home file to use for a controller action.
     *
     * To map multiple action homes in one go pass an array as first argument and no second argument.
     *
     * @param string|array $action Action or array of actions
     * @param string|null $home home name
     * @return void
     * @throws \Crud\Error\Exception\ActionNotConfiguredException
     * @throws \Crud\Error\Exception\MissingActionException
     */
    public function home($action, ?string $home = null): void
    {
        if (is_array($action)) {
            foreach ($action as $realAction => $realhome) {
                /** @psalm-suppress UndefinedMethod */
                $this->action($realAction)->home($realhome);
            }
        ...
        /** @psalm-suppress UndefinedMethod */
        $this->action($action)->home($home);
    }

    /**
     * Change the homeVar name for one or multiple actions.
     *
     * To map multiple action homeVars in one go pass an array as first argument and no second argument.
     *
     * @param string|array $action Action or array of actions.
     * @param string|null $homeVar home var name.
     * @return void
     * @throws \Crud\Error\Exception\ActionNotConfiguredException
     * @throws \Crud\Error\Exception\MissingActionException
     */
    public function homeVar($action, ?string $homeVar = null): void
    {
        if (is_array($action)) {
            foreach ($action as $realAction => $realhomeVar) {
                /** @psalm-suppress UndefinedMethod */
                $this->action($realAction)->homeVar($realhomeVar);
            }
        ...
        /** @psalm-suppress UndefinedMethod */
        $this->action($action)->homeVar($homeVar);
    }
